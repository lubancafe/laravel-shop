    ...
    <div class="box-body">
      <table class="table table-bordered">
        ...
        <tr>  
          <!-- 这里也新增了一个发货状态 -->
          <td>发货状态：</td>
          <td  colspan="3">@if($order->ship_status){{ \App\Models\Order::$shipStatusMap[$order->ship_status] }}@endif</td>
        </tr>
        <!-- 订单发货开始 -->
        <!-- 如果订单未发货，展示发货表单 -->
        @if(!$order->ship_status || $order->ship_status === \App\Models\Order::SHIP_STATUS_PENDING)
        <tr>
          <td colspan="4">
        ...
        <tr>
          <td>退款状态：</td>
          <td colspan="2">@if($order->ship_status){{ \App\Models\Order::$refundStatusMap[$order->refund_status] }}，理由：{{ $order->extra['refund_reason'] }} @endif</td>
          <td>
