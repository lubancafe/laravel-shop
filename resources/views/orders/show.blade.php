    ...
    <div class="order-bottom">
      <div class="order-info">
        ...
        <div class="line">
          ...
          <div class="line-value">@if($order->ship_status){{ \App\Models\Order::$shipStatusMap[$order->ship_status] }} @endif</div>
        </div>
        <!-- 如果有物流信息则展示 -->
        @if($order->ship_data)
        <div class="line">
        ...
        <div class="line">
          ...
          <div class="line-value">@if($order->refund_status) {{ \App\Models\Order::$refundStatusMap[$order->refund_status] }} @endif</div>
        </div>
        <div class="line">
      ...
      <div class="order-summary text-right">
        ...
        <a class="btn btn-primary btn-sm" href="{{ route('payment.alipay', ['order' => $order->id]) }}">Pay w. Alipay</a>
        </div>
        @endif
        <!-- 支付按钮结束 -->        

        <!-- 如果订单的发货状态为已发货则展示确认收货按钮 -->
        @if($order->ship_status && $order->ship_status === \App\Models\Order::SHIP_STATUS_DELIVERED)
...
{{--           <form method="post" action="{{ route('orders.received', [$order->id]) }}">
        ...
        @if($order->paid_at && ( !$order->refund_status || $order->refund_status === \App\Models\Order::REFUND_STATUS_PENDING))
